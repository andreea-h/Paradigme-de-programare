...
myConcat(List1, [], List1).
myConcat([], List2, List2).
myConcat([H|List1], List2, [H|List3]):- myConcat(List1, List2, List3).
...
myReverse([],[]).
myReverse([H|Rest], RevList):- myReverse(Rest, Right), myConcat(Right, [H], RevList).
%recursivitate de stiva
%rezultatul se construieste la revenirea din apelul recursiv
...
extract([], _, _, []).
%cazuri lista nevida
extract([H|_], 0, 0, [H]). %pune head-ul listei in rezultat
extract([H|Rest], 0, EndPos, [H|Result]):- EndPos1 is EndPos - 1, extract(Rest, 0, EndPos1, Result).
% daca indexul de start este mai mare decat 0, nu ne intereseaza head ul
% listei
extract([_|Rest], StartPos, EndPos, Result):- StartPos1 is StartPos - 1, EndPos1 is EndPos - 1,  extract(Rest, StartPos1, EndPos1, Result).

% daca indexul de start este mai mare decat 0, se apeleaza recursiv cu
% indecsii de start si end decrementati la fiecare apel, pana cand
% indexul de start devine 0 (nu ne intereseaza capul listei)
%
% cand indexul de start devine 0, se construieste recursiv lista
% rezultat decrementand indexul de final pana la 0
...
factorial2(0, 1).
factorial2(N, Rez):- factorial2(NRes, RezRes), N is NRes + 1, Rez is RezRes * N.
...
sublista([],[]).
sublista([H|Rest], [H|Snd_Rest]):-sublista(Rest, Snd_Rest).
sublista([_|Rest], Sublist) :- sublista(Rest, Sublist).
...
% verifica daca Node are asociat un nod si nu exista niciun arc de la
% Node la alt nod din graf
isLeaf(Node):-nod(Node), \+ arc(Node, _).
%Node e nod si nu se exista arc de la Node la orice altceva
%adica nu poate fi satisfacut scopul arc(Node, _).
...
%nu exista arc de la orice node la nodul Node
...
descendantOf(NodeX, NodeY):- nod(NodeX), nod(NodeY), arc(NodeY, NodeX).
