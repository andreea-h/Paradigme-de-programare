...
%se genereaza in template (care a fost instantiat initial)
%cu tot ce stim despre solutie) si se instantiaza template-ul
%a.i. ce am generat sa nu fie in dezacord cu ce s-a generat pana acum
...
%% safe(+X/Y/S, +Others)
safe(_, []):- !.
safe(X/Y/S,[HX/HY/Elem|Rest]):- ((S \== Elem);
                                (S == Elem, X \== HX, Y \== HY)), safe(X/Y/S, Rest).


% safe primeste un triplet si o lista si verifica daca tripletul este in
% acord cu lista care a fost deja instantiata
% adica daca valoarea din triplet este diferita de oricare valore din
% lista sau daca valorea din triplet, fiind egala cu o valoare din
% lista(adica adaugata deja in patrat), nu este pe acceasi linie/coloana
% in patrat cu aceasta
%
check1 :- tests([
...
search(Pb, [CurrentState|Other], Solution) :-
        ...
        \+ member(NextState, Other), %verifica daca starea tocmai generata 'NextState' nu a fost deja 'vizitata'
        search(Pb, [NextState,CurrentState|Other], Solution).

solve(Pb, Solution):-
        initial_state(Pb, State), %identifica in variabila state o stare initiala
        search(Pb, [State], Solution). %porneste cautarea dintr-o stare intiala
%si construieste solutia in 'Solution'
...
%metapredicate: au printre argumnete un predicat/scop
% findAll(template, goal, bag) - pentru fiecare varianta distincta de a
% satisface scopul ia din template si pune in lista bag

opus(est, vest).
opus(vest, est).

%ne arata ce configiratii ramase fara taran sunt ok;
safeTaran([]).
safeTaran([_]).
safeTaran(Cine) :- sort(Cine, [lup, varza]).
%combinatia [lup, varza] este singura valida
%capra+varza X
%capra+lup X

allTaran([capra, lup, varza]).

%Cine - cine se afla pe acelasi mal cu taranul
...
% forall primeste 2 scopuri si reuseste daca pentru fiecare varianta
% de a satisface scopul1 reuseste sa satisfaca scopul2
...
% călători cu barca.
% in barca se pot afla:
% 0 misionari, 2 canibali
% 0 misionari, 1 canibal
% 1 misionar, 1 canibal
% 1 misionar, 0 canibali
% 2 misionari, 0 canibali
boat(0, 2).
boat(0, 1).
boat(1, 1).
boat(1, 0).
...
% problemă.
safeMisionari(0, _):- !. %pe mal pot exista doar canibali
safeMisionari(NMisionari, NCanibali) :-(NMisionari >= NCanibali).
% pe un mal nu pot fi mai multi canibali decat misionari
...
%intial toti se afla pe malul est
...
% S1.
% parametru 2: starea din care se pleaca adica pentru care se cunosc
% toate campurile
% paramteru 3: stare in care se poate ajunge din starea curenta printr-o
% mutare valida de barca
%
next_state(misionari, CurrentState, NextState) :-
       parseState(CurrentState, Mal, NM_Est, NC_Est, NM_Vest, NC_Vest),
       % se genereaza o mutare valida
       %se verifica sa existe numarul de canibali, respeciv misionari pe care dorim sa ii mutam
       Mal == est,
       boat(Misionari, Canibali), Misionari =< NM_Est, Canibali =< NC_Est,
       %se calculeaza cati se vor afla pe fiecare mal dupa miscarea barcii
       NM_EstNou is NM_Est - Misionari, NM_VestNou is NM_Vest + Misionari, NC_EstNou is NC_Est - Canibali, NC_VestNou is NC_Vest + Canibali,
       safeMisionari(NM_EstNou, NC_EstNou), safeMisionari(NM_VestNou, NC_VestNou),
       NextState = state(vest, NM_EstNou, NC_EstNou, NM_VestNou, NC_VestNou).
...
next_state(misionari, CurrentState, NextState) :-
       ...
       %se verifica sa existe numarul de canibali, respeciv misionari pe care dorim sa ii mutam
       Mal == vest,
       boat(Misionari, Canibali), Misionari =< NM_Vest, Canibali =< NC_Vest,
       %se calculeaza cati se vor afla pe fiecare mal dupa miscarea barcii
       NM_EstNou is NM_Est + Misionari, NM_VestNou is NM_Vest - Misionari, NC_EstNou is NC_Est + Canibali, NC_VestNou is NC_Vest - Canibali,
       safeMisionari(NM_VestNou, NC_VestNou), safeMisionari(NM_EstNou, NC_EstNou),
       NextState = state(est, NM_EstNou, NC_EstNou, NM_VestNou, NC_VestNou).
