...
--iterate: genereaza o lista infinita prin aplicarea repetata a lui f
arithmetic :: Num a => a -> a -> [a]
arithmetic first ratia = iterate (+ ratia) first
geometric :: Num a => a -> a -> [a]
geometric first ratia  = iterate (* ratia) first
{-
  3. (2p)
...
--urmatorul numar generat este primul element din ceea ce intoarce next
randoms :: Int -> [Word8]
randoms seed = map (fromIntegral . fst) (iterate (next . snd) (next (mkStdGen seed)))


--(iterate (next. snd) (next $ mkStdGen seed)) intoarce o lista infinita de tupluri (Int, StdGen)
--(next (snd (next $ mkStdGen seed))) : intoarce un tuplu nou (nr_generat, stare_nou_generator) -- este echivalent cu a scrie (next . snd) (next $ mkStdGen seed)
--(snd $ next $ mkStdGen seed) : intoarce urmatoare stare a generatorului
--(next $ mkStdGen 2) : (ce se genereaza, urmatorul generator)
...
--chr( pos + ord 'a' )			
--		where 
--		pos = fromIntegral(mod word8 26)
		
			 
--(ord 'A')--> 65
--(ord 'a')--> 97 ;;codul ASCII asociat caracterului
--(chr 65)--->'A'
