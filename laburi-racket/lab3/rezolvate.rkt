...
(define (ins-in-sorted op) ;;se primeste ca argument un operator de comparatie
  (lambda (e L) 
    (if (or (null? L) (op e (car L)))     ;; op = un operator de comparatie
        (cons e L) ;;daca se respecta ordinea data prin 'op'
        (cons (car L) ((ins-in-sorted op) e (cdr L)))))) ;;se apeleaza functia ins-in-sorted pentru elementul e si restul listei L
...
(define (filter1 p L)
  (cond
    ((null? L) L)
    ((p (car L)) (cons (car L) (filter1 p (cdr L)))) ;p primeste un singur argument
