...
;(define-struct bird_str (x y current_y current_y_speed))
;(define get-initial-state (bird_str bird-x bird-initial-y 0))

(define-struct struct-bird (x y v-y) #:transparent)
(define-struct struct-pipe (x y gap_y) #:transparent)
(define-struct struct-variables (gravity momentum scrool-spedd) #:transparent)
(define my_bird (struct-bird bird-x bird-initial-y 0))
(define my_pipe (struct-pipe 50 50 30))
(define-struct struct-state (my_bird my_pipe struct-variables score) #:transparent)
...
(define (next-state-bird bird gravity)
  (struct-copy struct-bird bird [v-y (+ 2 gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy struct-bird bird [v-y momentum]))
